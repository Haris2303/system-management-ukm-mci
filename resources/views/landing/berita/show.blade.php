    ...
    <div class="pt-24 pb-4 bg-white border-b border-slate-100">
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            <nav class="flex items-center gap-2 text-sm text-slate-400">
                ...
                <a href="{{ route('berita.index') }}" class="hover:text-brand-600 transition-colors">Berita</a>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
                <span class="text-slate-600 font-medium truncate max-w-xs">{{ Str::limit($post->judul, 40) }}</span>
            </nav>
        </div>
    </div>
        ...
        <header class="py-12 bg-white">
            <div class="max-w-4xl mx-auto px-6 lg:px-8">
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6">
                    <a href="{{ route('berita.index', ['kategori' => $post->kategori]) }}"
                        class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-bold
               @switch($post->kategori)
                   @case('Prestasi')  bg-amber-50 text-amber-700 border border-amber-200 @break
                   @case('Kegiatan')  bg-emerald-50 text-emerald-700 border border-emerald-200 @break
               ...
               @endswitch">
                ...
                <div class="flex items-center justify-between flex-wrap gap-4 py-5 border-y border-slate-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="font-semibold text-slate-800 text-sm">{{ $post->author->name ?? 'Admin MCI' }}
                            </div>
                            <div class="text-slate-400 text-xs">
                                {{ $post->published_at->translatedFormat('l, d F Y · H:i') }} WIT</div>
                        </div>
                    </div>
                    {{-- Share buttons --}}
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-slate-400 font-medium">Bagikan:</span>
                        <a href="https://twitter.com/intent/tweet?text={{ urlencode($post->judul) }}&url={{ urlencode(request()->url()) }}"
                        ...
                        <dl class="space-y-3 text-sm">
                            ...
                            <div>
                                ...
                                <dd class="font-semibold text-slate-700">
                                    {{ $post->published_at->translatedFormat('d F Y') }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-400 text-xs mb-0.5">Penulis</dt>
                                <dd class="font-semibold text-slate-700">{{ $post->author->name ?? 'Admin MCI' }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-400 text-xs mb-0.5">Dibaca</dt>
                                <dd class="font-semibold text-slate-700">{{ number_format($post->views) }} kali</dd>
                            </div>
                            <div>
                                <dt class="text-slate-400 text-xs mb-0.5">Estimasi Baca</dt>
                                <dd class="font-semibold text-slate-700">{{ $post->readTime() }}</dd>
                            </div>
                        </dl>
                    </div>
