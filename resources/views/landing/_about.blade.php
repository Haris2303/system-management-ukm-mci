...
<section id="tentang" class="py-28 bg-canvas-soft relative overflow-hidden">
    ...
    <div
        class="absolute top-0 right-0 w-72 h-72 bg-brand-100/40 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl pointer-events-none">
    </div>
            ...
            <div class="relative reveal">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div
                        ...
                        <div
                            class="absolute bottom-0 left-0 w-32 h-32 bg-brand-200/20 rounded-full translate-y-1/2 -translate-x-1/2">
                        </div>
                        <div class="relative z-10">
                            ...
                            <p class="text-brand-200 text-sm mt-1" style="font-weight:300;">Berkembang bersama teknologi
                                selama 7 tahun</p>
                        </div>
                        <div class="relative z-10 flex gap-6 mt-4">
                            <div>
                                <div class="text-2xl"
                                    style="font-weight:300; font-feature-settings:'tnum'; letter-spacing:-0.42px;">7
                                </div>
                    ...
                    @foreach ([['fa-solid fa-lightbulb', 'text-amber-500', 'Inovasi', 'Mendorong ide kreatif dan solusi teknologi terdepan'], ['fa-solid fa-handshake', 'text-brand-500', 'Kolaborasi', 'Membangun tim solid lintas jurusan dan angkatan']] as [$iconClass, $iconColor, $title, $desc])
                        <div
                            class="reveal reveal-delay-{{ $loop->index + 1 }} rounded-xl bg-white border border-hairline p-6 tech-card shadow-sm">
                            <div class="text-3xl mb-3 {{ $iconColor }}"><i class="{{ $iconClass }}"></i></div>
                            <div class="text-ink text-base mb-1" style="font-weight:300; letter-spacing:-0.22px;">
                                {{ $title }}</div>
                            <div class="text-ink-mute text-sm leading-relaxed" style="font-weight:300;">
                                {{ $desc }}</div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="space-y-7">
                <div class="reveal">
                    ...
                    <h2 class="font-display text-4xl lg:text-5xl text-ink leading-tight"
                        ...
                        {{ $profil?->tagline ? implode(' ', array_slice(explode(' ', $profil->tagline), 0, -1)) : 'Komunitas yang' }}
                        <span class="gradient-text">
                            {{ $profil?->tagline ? implode(' ', array_slice(explode(' ', $profil->tagline), -1)) : 'Menginspirasi' }}
                        </span>
                    </h2>
                </div>
                ...
                <div class="space-y-4 text-ink-mute leading-relaxed reveal reveal-delay-1" style="font-weight:300;">
                    ...
                    @else
                        <p>
                            <strong class="text-ink-secondary" style="font-weight:400;">UKM MCI (Mahasiswa Creative &
                                Innovation)</strong>
                            adalah unit kegiatan mahasiswa yang berfokus pada pengembangan kompetensi di bidang
                            teknologi
                            informasi, pemrograman, kecerdasan buatan, dan desain digital.
                        </p>
                        <p>
                            Kami percaya bahwa setiap mahasiswa memiliki potensi untuk menjadi innovator. Melalui
                            berbagai
                            program, workshop, dan kompetisi, kami hadir sebagai wadah untuk mengembangkan skill,
                            membangun
                ...
                <div class="space-y-3 reveal reveal-delay-2">
                    ...
                    @foreach ($keunggulan as $item)
                        <div class="flex items-center gap-3">
                            <i
                                class="{{ $item['icon'] ?? 'fa-solid fa-circle-check' }} text-brand-600 w-4 text-center flex-shrink-0"></i>
