...
<div class="idc-info">
    ...
    <span>
        ...
        Sebelum mendesain, lihat <a href="{{ route('id-card.template') }}" target="_blank" style="font-weight:700;color:#1d4ed8;text-decoration:underline;">panduan zona &amp; template layout</a> agar desain tidak menutupi konten ID card.
    </span>
</div>
    ...
    @if($this->backgroundImage)
        <div class="idc-bg-body">
            <div class="idc-bg-thumb"
                 style="background-image: url('{{ $this->backgroundImage }}');"></div>
...
<div class="idc-grid">
    @foreach($this->getTemplates() as $slug => $tpl)
        @php $isActive = $slug === $this->activeTemplate && !$this->backgroundImage; @endphp
        ...
        >
            ...
            <div class="idc-swatch" style="background: {{ $tpl['preview_bg'] }};">
                <div class="idc-mock">
                    <div class="idc-mock-header" style="background: {{ $tpl['preview_bg'] }};"></div>
                    <div class="idc-mock-body">
                        <div class="idc-mock-avatar" style="background: {{ $tpl['preview_bg'] }};"></div>
                        <div class="idc-mock-lines">
                            <div class="idc-mock-line" style="width:70%; background:{{ $tpl['preview_bg'] }};opacity:.35;"></div>
                            <div class="idc-mock-line" style="width:45%; background:{{ $tpl['preview_bg'] }};opacity:.2;"></div>
                        </div>
                    </div>
                </div>
            ...
            {{-- Label --}}
            <div class="idc-label">
                <span class="idc-label-name {{ $isActive ? 'active' : '' }}">{{ $tpl['label'] }}</span>
                @if($isActive)
...
<div class="idc-footer">
    <a href="{{ route('id-card.preview') }}" target="_blank" class="idc-preview-btn">
    ...
    <span class="idc-footer-note">
        ...
        @else
            Template aktif:
            <strong>{{ \App\Support\IdCardTemplates::find($this->activeTemplate)['label'] }}</strong>
        @endif
    </span>
</div>
